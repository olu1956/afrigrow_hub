...
export function CreateQuotationForm({
  ...
  const total = useMemo(() => calculateQuotationTotal(parsedItems), [parsedItems]);
  ...
  async function handleSubmit(status: QuotationStatus) {
    ...
    if (!result.ok) {
      const message = result.error ?? "Could not save quotation.";
    ...
    <section className={dashboardCardClass}>
      ...
      <form onSubmit={onFormSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-between rounded-xl border border-border bg-background p-4">
          ...
          <span className="text-lg font-bold text-primary">{formatInvoiceMoney(total)}</span>
        </div>
